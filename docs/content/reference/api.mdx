...
A `CompositeRule` is a parse-time macro — one spelling that expands to a sub-pattern (e.g. strftime
`%T` → `%H:%M:%S`). Rendering produces the expansion, never the composite, so a composite normalizes
on a round trip.

```ts
interface CompositeRule {
  readonly token: string // the composite spelling, e.g. '%T'
  readonly expandsTo: string // its sub-pattern, in this dialect's grammar, e.g. '%H:%M:%S'
}
```
...
interface DelimitedSyntax {
  ...
  readonly open: string // e.g. '[' or "'"
  readonly close: string // e.g. ']' or "'"
  readonly escapedDelimiter?: string // a doubled quote, for quote-style literals
}

interface DirectiveSyntax {
  readonly kind: 'directive'
  readonly marker: string // e.g. '%'; doubling it is a literal marker
}

type TokenSyntax = DelimitedSyntax | DirectiveSyntax
```
